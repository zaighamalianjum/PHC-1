...
import {
  ...
  DollarSign, QrCode, Boxes, X, RotateCcw, Pencil, Eye, Lock, ScanLine
} from 'lucide-react';
...
interface PurchaseOrdersTabProps {
  ...
  handleDirectPrintGrn?: (grn: ErpGrn) => void;
}
...
export const PurchaseOrdersTab: React.FC<PurchaseOrdersTabProps> = ({
  ...
  setShowScanGrnModal = (_val?: any) => {},
  handleDirectPrintGrn,
}) => {
  return (
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex items-center space-x-2 self-start">
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white font-bold text-xs transition flex items-center space-x-1.5 shadow-sm cursor-pointer"
                  title="Scan Vendor GRN / Delivery Invoice Image with AI"
                >
                      ...
                      return (
                        ...
                        <td className="p-3 text-center">
                          <div className="flex items-center justify-center space-x-1.5">
                            <button
                              type="button"
                              onClick={() => handleDirectPrintGrn ? handleDirectPrintGrn(grn) : handleOpenGrnPrintPreview(grn)}
                              className="px-2.5 py-1 text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition cursor-pointer flex items-center space-x-1 font-bold shadow-xs text-[11px]"
                              title="Direct Print GRN Document straight to Printer (No preview popup)"
                            >
                              <Printer className="w-3.5 h-3.5" />
