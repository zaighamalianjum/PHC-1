...
export default function SettingsDesk({
  ...
  const MAIN_MENU_CONFIGS: MainMenuDefinition[] = [
    ...
    },
    {
      ...
      subMenus: [
        { key: 'canAccessErpOverview', label: 'ERP Dashboard & KPI Overview (Admin Only)', icon: PieChart, desc: 'Financial summaries, gross margins, cash position & revenue stats (Administrator only)' },
        { key: 'canAccessErpFiscalCalendar', label: 'Fiscal Year & Calendar Setup (Admin Only)', icon: Calendar, desc: 'Financial accounting periods, quarterly reviews & fiscal year closing (Administrator only)' },
        { key: 'canAccessErpCashBook', label: 'Clinic Cash Book & P&L Log (Admin Only)', icon: Landmark, desc: 'Real-time cash inflows, patient fees, expenses & profit/loss tracking (Administrator only)' },
        ...
        { key: 'canAccessErpLedger', label: 'Financial Ledger & Journal (Admin Only)', icon: Receipt, desc: 'Double-entry transaction audit trails & general ledger journals (Administrator only)' },
